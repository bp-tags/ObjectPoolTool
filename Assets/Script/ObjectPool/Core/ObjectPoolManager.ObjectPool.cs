        ...
        private sealed class ObjectPool<T> : ObjectPoolBase, IObjectPool<T> where T : ObjectBase
        {
            ...
            public override int Capacity
            {
                ...
                set
                {
                    if (value < 0)
                    {
                        throw new UnityEngine.UnityException("Capacity is invalid.");
                    }
                    ...
                    UnityEngine.Debug.Log(string.Format("Object pool '{0}' capacity changed from '{1}' to '{2}'.", Name, m_Capacity.ToString(), value.ToString()));
                    m_Capacity = value;
                    Release();
                }
            }
            ...
            public override float ExpireTime
            {
                ...
                set
                {
                    if (value < 0f)
                    {
                        throw new UnityEngine.UnityException("ExpireTime is invalid.");
                    }
                    ...
                    UnityEngine.Debug.Log(string.Format("Object pool '{0}' expire time changed from '{1}' to '{2}'.", Name, m_ExpireTime.ToString(), value.ToString()));
                    m_ExpireTime = value;
                    Release();
                }
            }
            ...
            public void Register(T obj, bool spawned)
            {
                if (obj == null)
                {
                    throw new UnityEngine.UnityException("Object is invalid.");
                }

                UnityEngine.Debug.Log(string.Format(spawned ? "Object pool '{0}' create and spawned '{1}'." : "Object pool '{0}' create '{1}'.", Name, obj.Name));
            ...
            public T Spawn(string name)
            {
                foreach (Object<T> obj in m_Objects)
                {
                    ...
                    if (m_AllowMultiSpawn || !obj.IsInUse)
                    {
                        UnityEngine.Debug.Log(string.Format("Object pool '{0}' spawn '{1}'.", Name, obj.Peek().Name));
                        return obj.Spawn();
                    }
                }
            ...
            public void Unspawn(object target)
            {
                if (target == null)
                {
                    throw new UnityEngine.UnityException("Target is invalid.");
                }
                ...
                foreach (Object<T> obj in m_Objects)
                {
                    ...
                    {
                        UnityEngine.Debug.Log(string.Format("Object pool '{0}' unspawn '{1}'.", Name, obj.Peek().Name));
                        obj.Unspawn();
                        Release();
                        return;
                    }
                ...
                throw new UnityEngine.UnityException(string.Format("Can not find target in object pool '{0}'.", Name));
            }
            ...
            public void SetLocked(object target, bool locked)
            {
                ...
                foreach (Object<T> obj in m_Objects)
                {
                    if (obj.Peek().Target == target)
                    {
                        UnityEngine.Debug.Log(string.Format("Object pool '{0}' set locked '{1}' to '{2}.", Name, obj.Peek().Name, locked.ToString()));
                        obj.Locked = locked;
                        return;
                    }
                }
            ...
            public void SetPriority(object target, int priority)
            {
                ...
                foreach (Object<T> obj in m_Objects)
                {
                    if (obj.Peek().Target == target)
                    {
                        UnityEngine.Debug.Log(string.Format("Object pool '{0}' set priority '{1}' to '{2}.", Name, obj.Peek().Name, priority.ToString()));
            ...
            public override void ReleaseAllUnused()
            {
                ...
                while (current != null)
                {
                    ...
                    UnityEngine.Debug.Log(string.Format("Object pool '{0}' release '{1}'.", Name, current.Value.Name));
                    current = next;
                }
            }
            ...
            internal override void Update(float elapseSeconds, float realElapseSeconds)
            {
                ...
                UnityEngine.Debug.Log(string.Format("Object pool '{0}' auto release start.", Name));
                Release();
                UnityEngine.Debug.Log(string.Format("Object pool '{0}' auto release complete.", Name));
            }
